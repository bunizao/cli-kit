...
function renderTable(value: unknown, columns?: readonly [string, string][]): string {
  const rows = Array.isArray(value) ? value : [value];
  if (rows.length === 0) return "";
  if (!rows.every(isRecord)) return `${rows.map(String).join("\n")}\n`;

  const selected = columns ?? Object.keys(rows[0] ?? {}).map((key) => [key, key] as const);
  const values = rows.map((row) => selected.map(([key]) => formatCell(row[key])));
  const widths = selected.map(([, label], index) =>
